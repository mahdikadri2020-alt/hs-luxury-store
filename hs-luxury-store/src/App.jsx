...
// --- Firebase Configuration (VS CODE SETUP) ---
...
const ALGERIA_DATA = {
  ...
  "15 Tizi Ouzou": ["Tizi Oوزو", "Azazga", "Draa El Mizan", "Tigzirt", "Boghni", "Larbaâ Nath Irathen", "Azeffoun", "Mekla", "Ouadhias", "Tizi Gheniff"],
  ...
  "23 Annaba": ["Annaba", "El Bouni", "El Hadjar", "Berrahal", "Seraïدي", "Chetaïbi", "Sidi Amar", "Ain Berda", "Tréat"],
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in">
      ...
      <div className="bg-white w-full max-w-md p-10 rounded-[2.5rem] shadow-2xl">
        ...
        <form onSubmit={handleLogin} className="space-y-4" dir="rtl">
          ...
          <button type="button" onClick={onCancel} className="w-full text-neutral-400 text-xs font-bold uppercase mt-2 text-center hover:text-neutral-600 transition">إلغاء</button>
...
const AdminPanel = ({ onLogout, products, orders, deliveryPrices, notify }) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          // Convert to JPEG with 0.5 quality to reduce size
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-[#F8F9FA] font-sans overflow-hidden">
      ...
      <aside className="w-full md:w-64 bg-[#1A1A1A] text-white flex flex-row md:flex-col justify-between md:justify-start items-center md:items-stretch p-4 md:p-6 shadow-xl sticky top-0 z-20 shrink-0 overflow-x-auto md:overflow-visible gap-4 md:gap-0 h-auto md:h-screen">

        <h2 className="text-lg md:text-xl font-serif tracking-widest text-amber-500 mb-0 md:mb-10 text-center uppercase tracking-[0.3em] whitespace-nowrap">H&S Admin</h2>
