...
const ALGERIA_DATA = {
  ...
  "35 Boumerdès": ["Boumerdès", "Boudouaou", "Dellys", "Khemis El Khechna", "Thénia", "Isser", "Zemmouri", "Corso", "Ouled Hedadj"],
 ...
 */
const AdminPanel = ({ onLogout, products, orders, deliveryPrices, notify, isConnected }) => {
  ...
  // Utility to compress image (Improved to ensure smaller size)
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        ...
        img.onload = () => {
          ...
          const MAX_WIDTH = 500; // Keep max width reasonable
          ...
          // Reduced quality to 0.5 to prevent Firestore limits (1MB)
  ...
  const handleSaveProduct = async (e) => {
    ...
    if (!auth.currentUser) {
        setFormError("Erreur: Vous n'êtes pas connecté au serveur.");
        return;
    }
    ...
    try {
      ...
      setEditingId(null); setActiveTab('products');
        ...
        // Show persistent error in form
        if (err.code === 'resource-exhausted' || err.toString().includes('size')) {
             setFormError("خطأ: حجم الصور كبير جداً. حاول استخدام صور أصغر.");
        } else if (err.code === 'permission-denied') {
             setFormError("خطأ: ليس لديك صلاحية التعديل (Permission Denied).");
        } else {
             setFormError("خطأ غير معروف: " + (err.message || "فشل الاتصال")); 
        }
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-[#F8F9FA] font-sans overflow-hidden">
      ...
      <aside className="w-full md:w-64 bg-[#1A1A1A] text-white flex flex-row md:flex-col justify-between md:justify-start items-center md:items-stretch p-4 md:p-6 shadow-xl sticky top-0 z-20 shrink-0 gap-4 md:gap-0 h-auto md:h-screen">
        ...
        <div className="mt-0 md:mt-10 pt-0 md:pt-6 border-t-0 md:border-t border-neutral-800">
            <div className={`flex items-center gap-2 mb-4 px-4 text-[10px] font-bold uppercase tracking-widest ${isConnected ? 'text-green-500' : 'text-red-500'}`}>
                {isConnected ? <Wifi size={14} /> : <WifiOff size={14} />} {isConnected ? 'Connecté' : 'Hors ligne'}
            </div>
            <button onClick={onLogout} className="flex w-full items-center gap-3 p-3 md:p-4 text-neutral-500 hover:text-red-400 transition font-bold text-center"><LogOut size={18} /> <span className="hidden md:inline">Quitter</span></button>
        </div>
      </aside>
        ...
        {activeTab === 'add' && (
          <div className="max-w-4xl mx-auto bg-white p-6 md:p-12 rounded-[3rem] shadow-xl text-left animate-in slide-in-from-bottom-4">
             ...
             <form onSubmit={handleSaveProduct} className="space-y-8">
               ...
               <button type="submit" disabled={isSubmitting || !isConnected} className="w-full bg-black text-white font-black py-6 rounded-[2rem] hover:bg-amber-900 transition flex items-center justify-center gap-4 text-xs text-center shadow-xl disabled:opacity-50 disabled:cursor-not-allowed">
               ...
               <table className="w-full text-left min-w-[600px]">
                 ...
                 <tbody className="divide-y divide-neutral-100">
                   ...
                   {sortedProducts.length === 0 && (
                       <tr><td colSpan="4" className="p-8 text-center text-neutral-400 text-sm">Aucun produit trouvé.</td></tr>
                   )}
                 </tbody>
               </table>
             </div>
...
export default function App() {
  ...
  const renderContent = () => {
    if (view === 'admin' && isLoggedIn) {
      return <AdminPanel onLogout={() => setView('landing')} products={products} orders={orders} deliveryPrices={deliveryPrices} notify={showToast} isConnected={!!user} />;
    }
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {products.length === 0 && (
                <div className="text-center py-20">
                    ...
                    <button onClick={() => setShowLogin(true)} className="mt-4 text-xs font-bold uppercase text-neutral-300 hover:text-neutral-500">Connexion Admin</button>
                </div>
            )}
          </div>
        </section>
